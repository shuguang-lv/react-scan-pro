...
test.describe("Overlay inspector flow", () => {
  ...
  test("inspector header shows the focused component name", async ({ page }) => {
    ...
    await expect(page.locator(`${TOOLBAR_SELECTORS.widget} >> text=Counter`).first()).toBeVisible();
  });
  ...
  test("a copy button appears when a component is focused", async ({ page }) => {
    ...
    await expect(page.locator('.react-scan-pro-close-button[title*="Copy element"]')).toBeVisible();
  });
  ...
  test("re-focusing a different component keeps the inspector open", async ({ page }) => {
    await enterInspectMode(page);
    await focusComponent(page, '[data-testid="increment"]');
    expect(await getInspectStateKind(page)).toBe("focused");
  ...
  test("toggling inspect button off while focused tears down the panel", async ({ page }) => {
    await enterInspectMode(page);
    await focusComponent(page, '[data-testid="increment"]');
