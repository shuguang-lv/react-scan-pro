...
test.describe("Overlay notifications panel", () => {
  ...
  test("panel renders the slowdown history chrome", async ({ page }) => {
    ...
    // unlike the "No Events" empty state which disappears if React Scan Pro
    // incidentally records a slow render (e.g. under CPU contention).
    await expect(
      page.locator(`${TOOLBAR_SELECTORS.widget} button[title="Clear all events"]`),
    ).toBeVisible();
  });

  test("clicking the notifications button again closes the panel", async ({ page }) => {
    const minimizedHeight = await widgetHeight(page);

    await notificationsButton(page).click();
    await expect.poll(async () => widgetHeight(page)).toBeGreaterThan(minimizedHeight + 50);

    await notificationsButton(page).click();
    await expect.poll(async () => widgetHeight(page)).toBeLessThan(minimizedHeight + 50);
  });
  ...
  test("a slow interaction is recorded and surfaced in the panel", async ({ page }) => {
    ...
    await expect
      .poll(async () => page.locator(`${TOOLBAR_SELECTORS.widget} >> text=No Events`).count(), {
        timeout: 10_000,
      })
      .toBe(0);
  });
