...
const program = new Command()
  .name("react-scan-pro")
  .description("React Scan Pro CLI")
  .version(VERSION);

program
  .command("init")
  .description("Set up React Scan Pro in your project")
  .option("-y, --yes", "skip confirmation prompts", false)
  .option("-c, --cwd <cwd>", "working directory", process.cwd())
  .option("--skip-install", "skip package installation", false)
  .action(async (opts) => {
    console.log(`\n${pc.magenta("[·]")} ${pc.bold("React Scan Pro")} ${pc.dim(`v${VERSION}`)}\n`);
    ...
    try {
      ...
      if (project.framework === "unknown") {
        ...
        console.log(pc.dim("  React Scan Pro supports Next.js, Vite, and Webpack projects."));
        console.log(
          pc.dim(
            "  Visit https://github.com/shuguang-lv/react-scan-pro#install for manual setup.\n",
          ),
        );
        process.exit(1);
      }
      ...
      if (project.framework === "next") {
        console.log(
          `  Router:          ${pc.cyan(project.nextRouterType === "app" ? "App Router" : "Pages Router")}`,
        );
      }
      ...
      if (project.hasReactScan) {
        console.log(pc.green("  React Scan Pro is already installed in package.json."));
      ...
      if (hasCodeChanges) {
        printDiff(relative(cwd, result.filePath), result.originalContent!, result.newContent!);
      ...
      if (!opts.skipInstall && !project.hasReactScan) {
        console.log(pc.dim("\n  Installing react-scan-pro...\n"));
        installPackages(["react-scan-pro"], project.packageManager, cwd);
        console.log();
      }
      ...
      if (!hasCodeChanges && project.hasReactScan) {
        console.log(pc.green("  React Scan Pro is already set up in your project.\n"));
        process.exit(0);
      }
      ...
      console.log();
      console.log(`${pc.green("  Success!")} React Scan Pro has been installed.`);
