...
import { DroppedFramesEvent, getComponentName, getEventSeverity, InteractionEvent } from "./data";
        ...
        >
          <span className={cn(["min-w-fit"])}>
            <ChevronRight
              ...
              className={cn(["text-[#A1A1AA] transition-transform", expanded ? "rotate-90" : ""])}
              size={14}
            />
          </span>
      ...
      {expanded && (
        <IndentedContent>
          {item.events
            ...
            .map((event) => (
              <SlowdownHistoryItem event={event} shouldFlash={shouldFlash(event.id)} />
            ))}
        </IndentedContent>
      )}
    </div>
