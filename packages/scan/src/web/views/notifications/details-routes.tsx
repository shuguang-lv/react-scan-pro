          ...
          <div className={cn(["flex flex-col items-start gap-y-4"])}>
            ...
            {notificationState.events.length !== 0 && (
              <p className={cn(["text-xs"])}>
                Click on an item in the <span className={cn(["text-purple-400"])}>History</span>{" "}
                list to get started
              </p>
            )}
            <p className={cn(["text-zinc-600 text-xs"])}>
              You don't need to keep this panel open for React Scan Pro to record slowdowns
            </p>
            <p className={cn(["text-zinc-600 text-xs"])}>
            ...
            <button
              onClick={() => {
                ...
                localStorage.setItem("react-scan-pro-notifications-audio", "true");
            ...
            >
              {notificationState.audioNotificationsOptions.enabled ? (
                <>
                  <span className="flex items-center gap-x-1">Disable audio alerts</span>
                </>
              ) : (
                <>
                  <span className="flex items-center gap-x-1">Enable audio alerts</span>
                </>
              )}
            </button>
          </div>
    ...
    case "other-visualization": {
      return (
        <TabLayout>
          ...
          >
            <OtherVisualization selectedEvent={notificationState.selectedEvent} />
          </div>
        </TabLayout>
      );
    }
...
const TabLayout = ({ children }: { children: ReactNode }) => {
  ...
  return (
    <div className={cn([`w-full h-full flex flex-col gap-y-2`])}>
      ...
      <div className={cn(["h-calc(100%-50px) flex flex-col overflow-y-auto px-3"])}>{children}</div>
    </div>
  );
};
