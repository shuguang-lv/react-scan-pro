...
export const NotificationTabs = ({ selectedEvent: _ }: { selectedEvent: NotificationEvent }) => {
  const { notificationState, setNotificationState, setRoute } = useNotificationsContext();
  return (
    <div className={cn(["flex w-full justify-between items-center px-3 py-2 text-xs"])}>
      <div className={cn(["bg-[#18181B] flex items-center gap-x-1 p-1 rounded-sm"])}>
        <button
          onClick={() => {
            setRoute({
      ...
      <Popover
        triggerContent={
          <button
            onClick={() => {
              setNotificationState((prev) => {
                ...
                localStorage.setItem(
                  "react-scan-pro-notifications-audio",
            ...
            className="ml-auto"
          >
            <div className={cn(["flex gap-x-2 justify-center items-center text-[#6E6E77]"])}>
              <span>Alerts</span>
