...
export const RenderExplanation = ({
  ...
  useLayoutEffect(() => {
    const res = localStorage.getItem("react-scan-pro-tip-shown");
    const asBool = res === "true" ? true : res === "false" ? false : null;
    if (asBool === null) {
      setTipIsShown(true);
      localStorage.setItem("react-scan-pro-tip-is-shown", "true");
      return;
    }
    if (!asBool) {
      setTipIsShown(false);
  ...
  return (
    <div className={cn(["w-full min-h-fit h-full flex flex-col py-4 pt-0 rounded-sm"])}>
      <div className={cn(["flex items-start gap-x-4 "])}>
        ...
        <div className={cn(["flex flex-col gap-y-1"])}>
          <div className={cn(["text-sm font-bold text-white overflow-x-hidden"])}>
            <div className="flex items-center gap-x-2 truncate">{selectedFiber.name}</div>
          </div>
        ...
        >
          <button
            onClick={() => {
              ...
              localStorage.setItem("react-scan-pro-tip-shown", "false");
            }}
            className={cn(["absolute right-2 top-2 rounded-sm p-1 hover:bg-[#18181B]"])}
          >
            <CloseIcon size={12} />
          </button>
          <div className={cn(["w-1 bg-[#d36cff]"])} />
          <div className={cn(["flex-1"])}>
            <div className={cn(["px-3 py-2 text-gray-100 text-xs font-semibold"])}>
              How to stop renders
            </div>
      ...
      {isMemoizable && (
        ...
        >
          <div className={cn(["w-1 bg-[#d36cff]"])} />
          <div className={cn(["flex-1"])}>
            <div className={cn(["px-3 py-2 text-gray-100 text-sm font-semibold"])}>
              No changes detected
            </div>
      ...
      <div className={cn(["flex w-full"])}>
            ...
            selectedFiber.changes.props
              ...
              .map((change) => (
                ...
                >
                  ...
                  <div className={cn([" text-[8px]  text-[#d36cff] pl-1 py-1 "])}>
        ...
        <div
          className={cn(["flex flex-col border border-[#27272A] border-l-0 overflow-hidden w-1/3"])}
        >
          <div
            className={cn([
              " text-[14px] font-semibold px-2 py-2 bg-[#18181B] text-white flex justify-center",
          ...
          {selectedFiber.changes.state.length > 0 ? (
            selectedFiber.changes.state
              ...
              .map((change) => (
                ...
                >
                  <span className={cn(["text-white "])}>index {change.index}</span>
                  <div className={cn(["rounded-full  text-[#d36cff] pl-1 py-1 text-[8px]"])}>
                    {change.count}/{selectedFiber.count}x
                  </div>
                </div>
              ))
            ...
            selectedFiber.changes.context
              ...
              .map((change) => (
                ...
                >
                  <span className={cn(["text-white "])}>{change.name}</span>
                  <div
                    className={cn(["rounded-full text-[#d36cff] pl-1 py-1 text-[8px] text-wrap"])}
                  >
                    {change.count}/{selectedFiber.count}x
                  </div>
                </div>
