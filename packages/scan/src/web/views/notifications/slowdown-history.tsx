  ...
  switch (event.kind) {
    case "interaction": {
      return (
        ...
        >
          <div className={cn(["w-4/5 flex items-center justify-start h-full gap-x-1.5"])}>
    ...
    case "dropped-frames": {
      return (
        ...
        >
          <div className={cn(["w-4/5 flex items-center justify-start h-full text-xs truncate"])}>
    ...
    switch (lastEvent.kind) {
        ...
        if (
          ...
          return [
            ...eventsWithoutLast,
            {
              ...
              timestamp: Math.max(...[...lastEvent.events, curr].map((e) => e.timestamp)),
            },
          ];
        }
      ...
      case "single": {
        ...
        if (
          ...
          curr.kind === "interaction" &&
          curr.type === "keyboard" &&
          lastEvent.event.componentPath.join("-") === curr.componentPath.join("-")
        ) {
          const eventsWithoutLast = prev.filter((e) => e !== lastEvent);
          return [
            ...eventsWithoutLast,
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div className={cn(["w-full flex justify-center"])}>Clear all events</div>
        </Popover>
      </div>
      <div className={cn(["flex flex-col px-1 gap-y-1"])}>
        {collapsedEvents.length === 0 && (
          <div className={cn(["flex items-center justify-center text-zinc-500 text-sm py-4"])}>
            No Events
          </div>
        )}
